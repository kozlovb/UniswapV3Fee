...
def aggregate_fee_by_hour_when_in_range(pool_id, token_0_decimal, token_1_decimal, priceA, priceB, year, month, day, hour, minutes, seconds, differenceInDays):
    ...
    fee_0_previous = int(fee_prices[0]["feeGrowthGlobal1X128"])
    for fee_price in fee_prices:
        ...
        if price > priceA and price < priceB:
            ...
            total_fee_1 = total_fee_1 + int(fee_price["feeGrowthGlobal1X128"]) - fee_0_previous
        fee_0_previous = int(fee_price["feeGrowthGlobal0X128"])
        fee_1_previous = int(fee_price["feeGrowthGlobal0X128"])   
    # add up the fee here
    return total_fee_0, total_fee_1